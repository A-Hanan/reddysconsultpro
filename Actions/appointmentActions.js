...
export const getUpcomingAppointments = async (user) => {
...
export const getAttendedAppointments = async (userId, userType) => {
  //console.log("at action=>", bookAppointmentValues);
  let response;
  await api
    .get(`/appointment/attended/${userId}/${userType}`)
    .then((res) => {
...
export const getOngoingAppointment = async (user) => {
  if (!user) {
    return;
  }
  ...
  let response;
  /****checking appointment which status is ongoing */
  console.log("getting ongoing appointments............................");
  await api
    .get(`/appointment/ongoing/${user?.id}/${user?.userType}`)
    .then(async (res) => {
      console.log("response from ongoing", res.data);
      if (res.data.length > 0) {
        response = res.data[0];
      } else {
        await api
          .post(
            `/appointment/today-specific/${user?.id}/${user?.userType}`,
            body
          )
          .then((res) => {
